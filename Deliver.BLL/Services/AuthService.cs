...
namespace Deliver.BLL.Services
{
    ...
    ) : IAuthService
    {
        ...
        private async Task<Result> SendOtpAsync(ApplicationUser user)
        {
            ...
            var emailBody = _builder.GenerateEmailBody("EmailConfirmation", new Dictionary<string, string>
            {
                { "{{name}}", user.FirstName },
                { "{{otp_code}}", otpCode },
        ...
        private async Task<Result> SendPasswordResetOtpAsync(ApplicationUser user)
        {
            ...
            var emailBody = _builder.GenerateEmailBody("ForgetPassword", new Dictionary<string, string>
            {
                { "{{name}}", user.FirstName },
                { "{{otp_code}}", otpCode },
                { "{{expiry_minutes}}", _otpExpiryMinutes.ToString() }
